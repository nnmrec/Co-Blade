function plotBladeGModulus(SIM, BLADE, WEB, OUT, Panel)

figTitle = [SIM.case ' effective shear modulus'];
...
for i = 1:BLADE.NUM_SEC
    ...
    for n = 1:Panel(i).Top.nPanels
        ...
        y = Panel(i).Top.y{n};
        z = Panel(i).Top.z{n};
        c = Panel(i).Top.G_eff(n) * convModulus;
    ...
    for n = 1:Panel(i).Bot.nPanels
        ...
        y = Panel(i).Bot.y{n};
        z = Panel(i).Bot.z{n};
        c = Panel(i).Bot.G_eff(n) * convModulus;
        patch(z, x, y, c, 'LineWidth', 0.01)
    end
    ...
    for n = 1:WEB.nWebs(i)
        ...
        y = Panel(i).Web.y{n};
        z = Panel(i).Web.z{n};
        c = Panel(i).Web.G_eff(n) * convModulus;
        patch(z, x, y, c, 'LineWidth', 0.01)    
    end
...
set(get(cb,'xlabel'), 'String', 'effective shear modulus, G_{eff} (GPa)');
view(50,20)
axis image
alpha(1)
