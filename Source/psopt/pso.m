...
k = 1;
fprintf(1,'\n\n');
fprintf(1,'iter       fEvals      fEvalsTotal     bestF \n');
fprintf(1,'-------------------------------------------- \n');
while k <= options.Generations && totalEvals <= options.MaxFunEvals
% for k = 1:itr
    ...
    state.fevals = sum(~state.OutOfBounds);
    totalEvals   = totalEvals + state.fevals;
...
%     tempchk = alpha <= 1.6 ;
    ...
    if strncmpi(options.Display,'diag',4)
        fprintf(1,'%4.0f      %6.0f      %6.0f        %9.3e\n', state.Generation, state.fevals, totalEvals, state.fGlobalBest(k));
    end
