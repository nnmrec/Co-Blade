function plotLaminaStrain(SIM, BLADE, WEB, MATS, Panel, LaminaSS, OUT)
...
%%
convStrain = 1e6;           % conversion factor for strains (converts m to micro-m)
...
%% 1st principal strains
if OUT.PLOT_E11
    figTitle = [SIM.case ' lamina 1st principal strains'];    
    ...
    for i = 1:NUM_SEC
        ...
        for j = 1:Panel(i).Top.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Top(j).e_11_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Top.matID{j}(k),:))
            end
        end
        ...
        for j = 1:Panel(i).Bot.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Bot(j).e_11_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Bot.matID{j}(k),:))
            end
        end
        ...
        for j = 1:nWebs(i)
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, s_w, LaminaSS(i).Web(j).e_11_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Web.matID{j}(k),:))
            end
        end
    ...
    for i = 1:3
        ...
        if i == 1
            zlabel(ha(i), '\epsilon_{11}, (\mu-m/m)')
        end
        xlim(ha(i), [0 1])
        ylim(ha(i), [0 1])
        title(ha(i), ttl(i))
    ...
    legend(matName(matID_used), 'Location', 'West', 'FontName', 'times', 'FontSize', 12)
    axis off
...
%% 2nd principal strains
if OUT.PLOT_E22
    figTitle = [SIM.case ' lamina 2nd principal strains'];    
    fig = figure('name', figTitle, ...
                 'color', 'white', ...
                 'units','normalized',...
                 'outerposition',[0.1 0.1 0.8 0.8]);
    ...
    for i = 1:NUM_SEC
        ...
        for j = 1:Panel(i).Top.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Top(j).e_22_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Top.matID{j}(k),:))
            end
        end
        ...
        for j = 1:Panel(i).Bot.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Bot(j).e_22_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Bot.matID{j}(k),:))
            end
        end
        ...
        for j = 1:nWebs(i)
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, s_w, LaminaSS(i).Web(j).e_22_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Web.matID{j}(k),:))
            end
        end
    ...
    for i = 1:3
        ...
        if i == 1
            zlabel(ha(i), '\epsilon_{22}, (\mu-m/m)')
        end
        xlim(ha(i), [0 1])
        ylim(ha(i), [0 1])
        title(ha(i), ttl(i));
...
if OUT.PLOT_E12
    figTitle = [SIM.case ' lamina principal shear strains'];    
    fig = figure('name', figTitle, ...
                 'color', 'white', ...
                 'units','normalized',...
                 'outerposition',[0.1 0.1 0.8 0.8]);
    ...
    for i = 1:NUM_SEC
        ...
        for j = 1:Panel(i).Top.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Top(j).e_12_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Top.matID{j}(k),:))
            end
        end
        ...
        for j = 1:Panel(i).Bot.nPanels  
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, x_c, LaminaSS(i).Bot(j).e_12_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Bot.matID{j}(k),:))
            end
        end
        ...
        for j = 1:nWebs(i)
            ...
            for k = 1:nLam
                hold on
                plot3(z_L, s_w, LaminaSS(i).Web(j).e_12_maxabs{k} .* convStrain, 'Color', cmap(Panel(i).Web.matID{j}(k),:))
            end
        end
